...
function Triangle(x, y, a, vx, vy, va, h, hr) {
  return {
    x:  x,  y:  y,  a:  a,
    vx: vx, vy: vy, va: va,
    h: h, hr: hr
...
function tick(currentTime) {
  ...
  for (let i = 0; i < 4 && triangles.length < 4096; ++i) {
    triangles.push(Triangle(
        -32 + random(64), 96 + random(48), random(360),
        .2 - random(.4),   -.1 - random(2.5), random(3),
        random(.5)+.5, .008 + random(.01)));
  }
...
function animateScene() {
  ...
  gl.uniform4fv(uGlobalColor, [1, 0.2, 0.05, 1.0]);
